...
- **個股資金流向（第 8 區塊）**：同一份 20 日窗口不分子類股，各法人別列出買超前 50 + 賣超前 50；加速度門檻降到 0.3 億/日（個股 20 日日均絕對值 p90 只有 1.26 億，沿用子類股的 1 億等於只有前 12% 判得到）；另有市值與「5 日買賣超 / 市值」欄，市值 = 報告日收盤 × 發行股數（`t187ap03_L` / `mopsfin_t187ap03_O`，覆蓋 1154/1162 檔，缺的都是 4 碼 ETF）
